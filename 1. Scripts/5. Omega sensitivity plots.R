...
for (f in unique(Folders)){
  env<-gsub(".*species\\+(.*)( w_).*", "\\1", f) ## getting the environmental variable name from the model file name
  wtot<-gsub(".*( w_)(.*)( cladTP2.1).*", "\\2", f) ## getting the used omega value from the model file name 
...
for(sp in names(df_list)){
  ...
  #p_df$omega<-as.character(p_df$omega) ## Making sure this is a  
