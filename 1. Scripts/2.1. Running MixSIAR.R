...
set.seed(180959778) ## I have sampled 1:10^9 once and use that as a seed. #sample(1:10^9,1)
...
### At Sections 1.1-1.6. uploading consumer, source and dscrimination files
...
mix[["Mod with lake w_0.23"]] <- load_mix_data(filename= paste0("Data clado_asellus TP2.0/Consumerdata_w_0.23newTP_2.0.csv"), 
                                                iso_names    = c("d2H"), 
                                                factors      = c("Lake"),
                                                fac_random   = c(TRUE),   
                                                fac_nested   = NULL,  
...
mix[["Mod with lake+species w_0.23"]] <- load_mix_data(filename= paste0("2. Data/Consumerdata_w_0.23newTP_2.0.csv"), 
                                          iso_names    = c("d2H"), 
...
for ( i in loop_env_vars){
    mix[[paste("Mod with lake+species_rand+", i, " w_0.23_randSlope", sep="")]] <- load_mix_data(filename= paste0("2. Data/Consumerdata_w_0.23newTP_2.0.csv"), 
                                                                                  iso_names    = c("d2H"),
                                                                                  factors      = c("species", "Lake"), 
                                                                                  fac_random   = c(TRUE, TRUE),   
...
for ( w in unique(wtot)){
  mix[[paste("Mod with lake+species_rand+PC1 w_", w, "_randSlope", sep="")]] <- load_mix_data(filename= paste0("2. Data/Consumerdata_w_",w,"newTP_2.0.csv"), 
                                                                     iso_names    = c("d2H"), 
                                                                     factors      = c("species", "Lake"),
                                                                     fac_random   = c(TRUE, TRUE),   
                                                                     fac_nested   = c(FALSE, FALSE),
...
for (mod in unique(c(n.mod[6:7],n.mod[1:2]))){  
